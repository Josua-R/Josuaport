...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const socialLabels = {
    "twitter.com": "Follow on X",
    "instagram.com": "Follow on Instagram",
    "facebook.com": "Follow on Facebook",
...
(function setupBackgrounds() {
  ...
  function setBackground() {
    ...
    if (isIndexPage()) {
      ...
      const space = isMobile ? mobileBackgrounds : desktopspace;
      const randomIndex = Math.floor(Math.random() * space.length);
      target.src = space[randomIndex];
...
function initSimpleGallery() {
  ...
  const categoryPriority = ["Animation - complex", "Background art", "Illustration and fanart", "Storyboard", "Emoji", "Animation - simple"];
  ...
  sgTabs.innerHTML = categories
    .map(
      ...
      <li class="nav-item" role="presentation">
        <button class="nav-link2 ${i ? "" : "active"}"
                id="${slug(c)}-tab"
                data-bs-toggle="pill"
